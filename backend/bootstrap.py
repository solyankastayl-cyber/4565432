#!/usr/bin/env python3
"""
TA Engine + Exchange Intelligence Bootstrap System v3
======================================================

Быстрое развёртывание полной системы:

TA ENGINE:
- MongoDB коллекции
- Исторические данные (BTC, SPX, DXY)
- Calibration конфиг (Phase 8.6)
- Strategy registry (Phase 8.8)
- Regime activation map (Phase 8.9)
- Cross-asset baseline (Phase 9.0)
- Alpha Node Registry (Phase 13.1)

EXCHANGE INTELLIGENCE (Phase 13.8.B):
- Funding context data
- OI snapshots
- Liquidation events
- Order flow data
- Symbol snapshots (derivatives)
...
class Bootstrap:
    ...
    def init_ta_config(self):
        ...
        configs = [
            ("calibration", CALIBRATION_CONFIG, "Phase 8.6"),
    ...
    def seed_exchange_data(self):
        ...
        print("\n📡 Exchange Data Seeding...")
        
        # Get symbols that have candles
        ...
        for symbol in EXCHANGE_SYMBOLS:
            ...
            if symbol not in symbols_with_candles:
                print(f"  ⚠ {symbol}: no candles, skipping exchange data")
                continue
            ...
            # Get candles for deriving exchange data
    ...
    def _seed_oi(self, symbol: str, candles: List[Dict]):
        ...
        base_oi = {"BTC": 15e9, "ETH": 8e9, "SOL": 2e9, "SPX": 5e9, "DXY": 1e9}.get(symbol, 1e9)
        oi_value = base_oi
    ...
    def _seed_snapshot(self, symbol: str, candles: List[Dict]):
        """Generate current derivatives snapshot"""
        if not candles:
            return
    ...
    # ═══════════════════════════════════════════════════════════
    # PHASE 28.1 — Microstructure Snapshot Seeding
    # ═══════════════════════════════════════════════════════════

    def init_microstructure_collections(self):
        """Create Microstructure Intelligence collections"""
    ...
    def seed_microstructure_data(self):
        """Seed microstructure snapshots from existing exchange data."""
        print("\n🔬 Microstructure Snapshot Seeding...")
        ...
        for symbol in EXCHANGE_SYMBOLS:
            ...
            # Build liquidation index by hour
            liq_by_ts = {}
            for ld in liq_docs:
                ts = ld.get("timestamp")
                if ts is None:
            ...
            for i in range(n):
                ...
                if liq_total > 0:
                    liq_pressure = round((liq_data["SHORT"] - liq_data["LONG"]) / liq_total, 4)
                else:
                    liq_pressure = round(random.uniform(-0.15, 0.15), 4)
                ...
                if oi_prev > 0:
                    oi_change = (oi_cur - oi_prev) / oi_prev
                    oi_pressure = round(min(max(oi_change / 0.10, -1.0), 1.0), 4)
                else:
                    oi_pressure = 0.0
                ...
                if liq_state == "THIN":
                    ...
                    ms_state = "SUPPORTIVE" if stress < 0.50 else "SUPPORTIVE"
                ...
                confidence = round(min(max(
                    0.25 * (1.0 - norm_spread)
                    + 0.25 * depth_score
                    + 0.20 * abs(imbalance_score)
                    + 0.15 * abs(liq_pressure)
    ...
    def status(self):
        ...
        print("\n" + "=" * 60)
        print("TA ENGINE + EXCHANGE INTELLIGENCE STATUS")
        print("=" * 60)
        ...
        print("│ ⚙️  Config:")
        for cfg in ["calibration", "coinbase", "binance", "exchange_intel"]:
        ...
        print("│")
        print("├─ EXCHANGE INTELLIGENCE (Phase 13.8.B) ──────────────────┤")
        ...
        binding_status = True
        for col_name, label in exchange_cols:
            count = self.db[col_name].count_documents({})
            symbols = self.db[col_name].distinct("symbol")
            status = "native" if count > 0 else "empty"
            if count == 0:
                binding_status = False
            print(f"│    {label}: {count:,} ({', '.join(symbols) if symbols else 'none'})")
        
        print(f"│")
        print(f"│ 🔗 Binding Status: {'NATIVE' if binding_status else 'PARTIAL'}")
        
        # Microstructure Intelligence
        print("│")
        print("├─ MICROSTRUCTURE INTELLIGENCE (Phase 28.1) ───────────────┤")
        
        ms_count = self.db.microstructure_snapshot_history.count_documents({})
        ms_symbols = self.db.microstructure_snapshot_history.distinct("symbol")
        print(f"│    Snapshots: {ms_count:,} ({', '.join(ms_symbols) if ms_symbols else 'none'})")
        
        for sym in ms_symbols:
            sym_count = self.db.microstructure_snapshot_history.count_documents({"symbol": sym})
            print(f"│      {sym}: {sym_count:,}")
    ...
    def run_exchange(self):
        """Bootstrap Exchange Intelligence only"""
        print("\n" + "=" * 60)
        print("EXCHANGE INTELLIGENCE BOOTSTRAP (Phase 13.8.B)")
        print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description="TA Engine + Exchange Intelligence Bootstrap",
