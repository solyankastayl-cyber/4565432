"""
Hypothesis Visualization API — PHASE 48.4
...
class HypothesisVisualizationService:
    """Service for hypothesis visualization."""
    ...
    def build_hypothesis_visualization(
        ...
        """Build visualization for a hypothesis."""
        ...
        # Analyze market for hypothesis
        direction, confidence, strength = self._analyze_direction(candles)
        
        # Build scenarios
        scenarios = self._build_scenarios(
            candles, current_price, current_time, direction, confidence
        )
        ...
        return HypothesisVisualization(
            ...
            alpha_contributors=[
                {"name": "Trend", "weight": 0.35, "signal": direction},
                {"name": "Momentum", "weight": 0.25, "signal": "aligned"},
                {"name": "Structure", "weight": 0.20, "signal": "supportive"},
                {"name": "Volume", "weight": 0.20, "signal": "confirming"},
            ],
        )
    ...
    def _analyze_direction(
        ...
        # Determine direction
        if ema_20[-1] > ema_50[-1] and momentum > 0.01:
            direction = "bullish"
            confidence = min(0.5 + abs(momentum) * 5, 0.85)
        elif ema_20[-1] < ema_50[-1] and momentum < -0.01:
            direction = "bearish"
            confidence = min(0.5 + abs(momentum) * 5, 0.85)
        else:
            direction = "neutral"
    ...
    def _build_scenarios(
        ...
        # Generate scenarios based on direction
        if direction == "bullish":
            # Base scenario
            scenarios.append(self._create_scenario(
                "base", current_price, current_time, hours_ahead,
                target_pct=0.03, probability=0.45, color="#22C55E", atr=atr
            ))
            
            # Bull scenario
            scenarios.append(self._create_scenario(
                "bull", current_price, current_time, hours_ahead,
                target_pct=0.06, probability=0.25, color="#10B981", atr=atr
            ))
            
            # Bear scenario (counter)
            scenarios.append(self._create_scenario(
                "bear", current_price, current_time, hours_ahead,
                target_pct=-0.02, probability=0.20, color="#EF4444", atr=atr
            ))
            
            # Extreme bull
            scenarios.append(self._create_scenario(
                "extreme_bull", current_price, current_time, hours_ahead,
                target_pct=0.10, probability=0.10, color="#059669", atr=atr
            ))
            ...
            scenarios.append(self._create_scenario(
                "base", current_price, current_time, hours_ahead,
                target_pct=-0.03, probability=0.45, color="#EF4444", atr=atr
            ))
            
            # Bear scenario
            scenarios.append(self._create_scenario(
                "bear", current_price, current_time, hours_ahead,
                target_pct=-0.06, probability=0.25, color="#DC2626", atr=atr
            ))
            
            # Bull scenario (counter)
            scenarios.append(self._create_scenario(
                "bull", current_price, current_time, hours_ahead,
                target_pct=0.02, probability=0.20, color="#22C55E", atr=atr
            ))
            
            # Extreme bear
            scenarios.append(self._create_scenario(
                "extreme_bear", current_price, current_time, hours_ahead,
                target_pct=-0.10, probability=0.10, color="#991B1B", atr=atr
            ))
            ...
            scenarios.append(self._create_scenario(
                "base", current_price, current_time, hours_ahead,
                target_pct=0.0, probability=0.50, color="#6B7280", atr=atr
            ))
            
            scenarios.append(self._create_scenario(
                "bull", current_price, current_time, hours_ahead,
                target_pct=0.02, probability=0.25, color="#22C55E", atr=atr
            ))
            
            scenarios.append(self._create_scenario(
                "bear", current_price, current_time, hours_ahead,
                target_pct=-0.02, probability=0.25, color="#EF4444", atr=atr
            ))
